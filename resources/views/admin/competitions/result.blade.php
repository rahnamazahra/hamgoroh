            ...
            <div class="card-header">
                <div class="card-title">
                    <div class="position-relative my-1">
                        <form method="GET" action="{{ route('admin.competitions.result', ['competition' => $competition->id]) }}">
                ...
                <div class="table-responsive">
                    <table id="permissions_list" class="table table-striped gy-7 gs-7">
                        ...
                        @forelse ($participants as $key => $participant)
                            @php
                                $row_number = ($participants->currentPage() -1) * ($participants->perPage()) + ($key + 1);
                                @endphp
                            <tr>
                                <td class="text-center">{{ $row_number }}</td>
                                <td class="text-center">{{ $participant->user->fullName() }}</td>
                                <td class="text-center">{{ $participant->user->national_code }}</td>
                                <td class="text-center">{{ $participant->user->province->title ?? ''}}</td>
