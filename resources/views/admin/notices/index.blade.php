...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-dark">اطلاعیه‌ها</li>
    </ul>
@endsection
        ...
        <div class="card shadow-sm">
            <div class="card-header">
                ...
                <div class="card-toolbar">
                    @can('notices-create')
                        <a href="{{ route('admin.notices.create') }}" class="btn btn-sm btn-primary">+ اطلاعیه جدید</a>
                    @endcan
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="permissions_list" class="table table-striped gy-7 gs-7">
                        ...
                        <tr>
                            ...
                            <th class="text-center">مخاطبان</th>
                        ...
                        @forelse ($notices as $key => $notice)
                            @php
                                $row_number = ($notices->currentPage() -1) * ($notices->perPage()) + ($key + 1);
                            @endphp
                            <tr>
                                <td class="text-center">{{ $row_number }}</td>
                                <td class="text-center">{{ $notice->title }}</td>
                                <td class="text-center">{!! $notice->description !!}</td>
                                <td class="text-center">
                                    @if($notice->is_sent_users == 1)
                                        <span class="badge badge-light-success">کاربران</span>
                                    @endif
                                    @if($notice->is_sent_referees == 1)
                                        <span class="badge badge-secondary">داوران</span>
                                    @endif
                                    @if($notice->is_sent_generals == 1)
                                        <span class="badge badge-light-info">مدیران کل</span>
                                    @endif
                                    @if($notice->is_sent_provincials == 1)
                                        <span class="badge badge-light-primary">مدیران استانی</span>
                                        @endif
                                        @if(json_decode($notice->selected_users))
                                            <span class="badge badge-light-warning">انتخابی</span>
                                    @endif
                                </td>
                                <td class="text-center">
                                    <div class="btn btn-group-sm">
                                        ...
                                        @can('notices-delete')
                                            <button name="btn_delete_item"
                                                    ...
                                                    data-id="{{ $notice->id }}"
                                                    data-url="{{ route('admin.notices.delete', ['notice' => $notice->id]) }}"
