...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-dark">معیار نمره دهی</li>
    </ul>
@endsection
        ...
        <div class="card shadow-sm">
            <div class="card-header">
                ...
                <div class="card-toolbar">
                    @can('criteria-create')
                        <a href="{{ route('admin.criteria.create') }}" class="btn btn-sm btn-primary">+ معیار جدید</a>
                    @endcan
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="permissions_list" class="table table-striped gy-7 gs-7">
                        ...
                        @forelse ($criteria as $key => $item)
                            @php
                                $row_number = ($criteria->currentPage() -1) * ($criteria->perPage()) + ($key + 1);
                            @endphp
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn btn-group-sm">
                                        @can('criteria-update')
                                            <a href="{{ route('admin.criteria.edit', ['criteria' => $item->id]) }}" class="btn btn-icon btn-bg-light btn-active-color-success btn-sm me-1" data-bs-toggle="tooltip" data-bs-placement="bottom" title="ویرایش">
                                        ...
                                        @endcan
                                        @can('criteria-delete')
                                            <button name="btn_delete_item" class="btn btn-icon btn-bg-light btn-active-color-danger btn-sm me-1" data-id="{{ $item->id }}" data-url="{{ route('admin.criteria.delete', ['criteria' => $item->id]) }}" data-bs-toggle="tooltip" data-bs-placement="bottom" title="حذف">
                                        <span class="svg-icon svg-icon-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div class="card-footer">
                {{ $criteria->links('pagination::bootstrap-5') }}
            </div>
        </div>
    @endcan
@endsection
