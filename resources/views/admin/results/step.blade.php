    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-muted">
            <a href="{{ route('admin.competitions.show', ['competition' => $competition->id]) }}"
               class="text-muted text-hover-primary">مدیریت دوره‌ها</a>
        </li>
        <li class="breadcrumb-item">
            <span class="bullet bg-gray-300 w-5px h-2px"></span>
        </li>
            ...
            <div class="card-header">
                <div class="card-title">
                    <div class="position-relative my-1">
                        <form method="GET"
                              action="{{ route('admin.stepResult.index', ['competition' => $competition, 'step' => $step->id]) }}">
...
{{--                                @foreach($i->evaluation->evaluationReferees as $j)--}}
                            <th class="text-center">{{ $i->evaluation->criteria->title . ' ' . $i->referee->last_name }}</th>
{{--                                @endforeach--}}
                            @endforeach
{{--                            <th class="text-center">داور</th>--}}
{{--                            <th class="text-center">نمره</th>--}}
                            <th class="text-center">نمره نهایی</th>
                        </tr>
                        </thead>
                        <tbody>
                        @forelse ($examiners as $key => $result)
{{--                                @php--}}
{{--                                    $row_number = ($examiners->currentPage() -1) * ($examiners->perPage()) + ($key + 1);--}}
{{--                                @endphp--}}
                                <tr>
{{--                                    <td class="text-center">{{ $row_number }}</td>--}}
                                    ...
                                    @php
                                        $scores = \App\Models\Result::where('step_id', $step->id)->where('examiner_id', $result->id)->get();
                                    @endphp
                                    ...
                                    <td class="text-center">{{ $score->score }}</td>
                                        ...
                                        @endif
                                    @endforeach
{{--                                    <td class="text-center">{{ $result->evaluation->criteria->title }}</td>--}}
{{--                                    <td class="text-center">{{ $result->referee->first_name . ' ' . $result->referee->last_name }}</td>--}}
