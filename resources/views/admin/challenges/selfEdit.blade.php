...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-dark">ویرایش زیررشته‌</li>
    </ul>
@endsection
...
@section('content')
    ...
    <div class="card shadow-sm">
        <form method="POST"
              action="{{ route('admin.challenges.selfUpdate', ['competition' => $competition->id, 'challenge' => $challenge->id]) }}">
            @method('PATCH')
            @csrf
                    ...
                    <div class="col-md-4 fv-row">
                        ...
                        <select class="form-select form-select-solid" data-control="select2"
                            ...
                            @foreach ($competition->ages as $age)
                                <option
                                    value="{{ $age->id }}" @selected($challenge->age_id == $age->id)>{{ $age->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 fv-row">
