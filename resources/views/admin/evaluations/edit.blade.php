...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-dark"> ویرایش داور</li>
    </ul>
@endsection

@section('content')
    <div class="card shadow-sm col-xl-12">
        <form method="POST" action="{{ route('admin.evaluations.update', ['evaluation' => $evaluation->id, 'step' => $step->id]) }}">
            @method('PATCH')
            @csrf
            <div class="card-header">
                <div class="card-title">ویرایش داور</div>
            </div>
            <div class="card-body">
                <div class="row g-9">
                    <div class="col-md-12 row">
                        <div class="col-md-6 fv-row my-5">
                            ...
                            <select class="form-select form-select-solid" data-control="select2" data-placeholder="لطفا انتخاب کنید" id="criteria_id" name="criteria_id">
                                ...
                                @foreach ($criterias as $criteria)
                                    <option value="{{ $criteria->id }}" @selected(old('criteria_id') == $criteria->id OR $evaluation->criteria_id == $criteria->id)>{{ $criteria->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 fv-row my-5">
                            ...
                            <select class="form-select form-select-solid" data-control="select2" data-placeholder="لطفا انتخاب کنید" id="refereeing_type" name="refereeing_type">
                                <option></option>
                                <option value="first" @selected(old('refereeing_type') == $evaluation->refereeing_type OR $evaluation->refereeing_type == 'first')>فردی</option>
                                <option value="average" @selected(old('refereeing_type') == $evaluation->refereeing_type OR $evaluation->refereeing_type == 'average')>گروهی</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-12 row">
                        <div class="col-md-6 fv-row my-5">
                            ...
                            <select class="form-select form-select-solid" data-control="select2" data-placeholder="لطفا انتخاب کنید" id="referee_id" name="referees[]" multiple="multiple">
                                ...
                                @foreach ($referees as $referee)
                                    <option value="{{ $referee->id }}" @selected((old('referees') and in_array($referee->id, old('referees'))) or $evaluation->evaluationReferees()->where('referee_id', $referee->id)->exists())> {{ $referee->first_name .' '. $referee->last_name }} </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 fv-row my-5">
                            <label for="point" class="form-label"> نمره </label>
                            <input type="text" class="form-control form-control-solid" id="point" name="point" value="{{ old('point', $evaluation->point) }}"/>
                        </div>
                    </div>
                </div>
            </div>
