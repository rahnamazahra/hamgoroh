...
@section('breadcrumb')
    <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">ویرایش مراحل </h1>
...
@section('content')
    ...
    <div class="card shadow-sm">
        <form method="POST"
              action="{{ route('admin.steps.selfUpdate', ['competition' => $competition->id, 'step' => $step->id]) }}">
            @method('PATCH')
            @csrf
                ...
                <div class="row g-9 my-2">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="" name="level" data-control="select2" data-placeholder="لطفا انتخاب کنید">
                            <option></option>
                            <option value="provincial" @selected($step->level == "provincial")>استانی</option>
                            <option value="country" @selected($step->level == "country")>کشوری</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="" name="type" data-control="select2" data-placeholder="لطفا انتخاب کنید">
                            <option></option>
                            <option value="video_upload" @selected($step->type == "video_upload")>آپلود ویدئو</option>
                            <option value="image_upload" @selected($step->type == "image_upload")>آپلود عکس</option>
                            <option value="voice_upload" @selected($step->type == "voice_upload")>آپلود صوت</option>
                            <option value="document_upload" @selected($step->type == "document_upload")>آپلود سند</option>
                            <option value="text" @selected($step->type == "text")>متن</option>
                            <option value="call" @selected($step->type == "call")>تماس تلفنی</option>
                            <option value="test" @selected($step->type == "test")>آزمون آنلاین چهارگزینه ای</option>
                            <option value="video_online" @selected($step->type == "video_online")>آزمون آنلاین تصویری</option>
                        </select>
                    </div>
                    <div class="col-md-1">
